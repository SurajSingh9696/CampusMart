...
export default function SellerAnalyticsPage() {
    ...
    fetch("/api/seller/analytics")
      ...
      .catch(() => {
        ...
        setData({
          ...
          viewsTotal: 1423,
          topListings: [
            { title: "Python DS/ML Notes", views: 412, orders: 18, revenue: 45000 },
            { title: "Circuit Design Kit", views: 298, orders: 9, revenue: 36000 },
            { title: "Hackathon Ticket", views: 241, orders: 7, revenue: 21000 },
          ],
          revenueByMonth: [
  ...
  return (
    <div className="space-y-7">
      ...
      {/* KPI cards */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Total Revenue" value={`₹${(data.totalRevenue / 100).toFixed(0)}`} icon="payments" color="#16a34a" bg="#f0fdf4" delay={0} />
          ...
          {data.revenueByMonth.map((m, i) => {
            ...
            return (
              <motion.div key={m.month} className="flex-1 flex flex-col items-center gap-2"
                ...
                <span className="text-[10px] font-bold text-slate-500">₹{(m.revenue / 100).toFixed(0)}</span>
          ...
          {data.topListings.map((l, i) => (
            <motion.div key={l.title} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.06 }}
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-slate-400 text-xs">{l.views} views · {l.orders} orders</p>
              </div>
              <p className="text-slate-900 font-black text-sm shrink-0">₹{(l.revenue / 100).toFixed(0)}</p>
            </motion.div>
          ))}
        </div>
      </div>
